...
export async function generateDependencyLicenses(mods: ModuleInfos): Promise<string> {
  const licenses = await Promise.all(Object.entries(mods).map(async ([name, pkg]) => {
    ...
    if( pkg.copyright ) license += `<span class="copyright">${pkg.copyright}</span>`;
    if( pkg.publisher ) license += `<span class="publisher">${pkg.publisher}</span>`;
    if( pkg.repository ) license += `<span class="homepage"><a href="${pkg.repository}">${pkg.repository}</a></span>`;
    if( licenseFile ) {
