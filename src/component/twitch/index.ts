...
function tokenReceiveSuccessful(
  res: TwitchOAuth2TokenResponse | TwitchOAuth2TokenErrorResponse,
): res is TwitchOAuth2TokenResponse {
...
export class Twitch extends Component<TwitchParameters> {
  ...
  async fetchTokenByDeviceCode(
    device_code: string,
  ): Promise<TwitchOAuth2TokenResponse | TwitchOAuth2TokenErrorResponse> {
    const obj = {
      client_id: clientId,
      scopes: this.scopes(),
      device_code,
